...
# ── Sensor constants ─────────────────────────────────────────────────────────
DHT_SENSOR_TYPE   = None   # will be set to Adafruit_DHT.DHT11 if available
...
class SensorManager:
    ...
    def _read_dht11(self):
        ...
        humidity, temperature = Adafruit_DHT.read_retry(
            DHT_SENSOR_TYPE, PIN_DHT11, retries=5, delay_seconds=0.5
        )
        if humidity is None or temperature is None:
            logger.warning("DHT11 read failed (None returned).")
            return None, None
        # Clamp to physically plausible range
        if not (0 <= temperature <= 60 and 0 <= humidity <= 100):
            logger.warning("DHT11 out-of-range: T=%.1f H=%.1f", temperature, humidity)
            return None, None
    ...
    def _poll_loop(self):
        ...
        while self._running:
            ...
            # ── Ultrasonic (every ULTRASONIC_POLL_INTERVAL) ────────────────
